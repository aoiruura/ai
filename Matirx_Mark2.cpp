...
template<class T> void operator *= (Matrix<T>& A, Matrix<T> B) {
	Matrix<T> ret(A.size(), B[0].size());
	for (int i = 0; i < A.size(); ++i) {
		for (int j = 0; j < B[0].size(); ++j) {
			for (int k = 0; k < B.size(); ++k) {
...
//掛け算の定義ミス発見 7/10 -> 修正 7/10

template<class T> int sgn(vector<T> A) {
	int cnt = 0;
...
template<class T> D det(Matrix<T> A) {
	D ret = 0;
	vector<int> sigma(A.size());
